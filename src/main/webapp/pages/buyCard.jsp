...
<body ontouchstart>
	...
	<div class="weui-flex p-t-15 p-b-15 weui-payselect">
		...
		<c:forEach var="sku" items="${skuList }">
			<div class="weui-flex__item weui-payselect-li">
				...
				<input type="hidden" value="${sku.id }-${sku.fv}">
			</div>
		</c:forEach>
	...
	<div class="weui-cells weui-cells_form m-t-0">
		...
		<div class="weui-cell" style="display: none;">
			<div class="weui-cell__hd">
				<label class="weui-label">自定义卡号</label>
			</div>
			<div class="weui-cell__bd">
		...
		<a class="weui-cell weui-cell_access open-popup" href="javascript:;"
			data-target="#fullpage" style="display: none;">
			<div class="weui-cell__bd">
				<p>选择卡面</p>
			</div>
			...
			<div class="weui-cell__ft">
				<div class="weui-count">
					<a class="weui-count__btn weui-count__decrease"></a> <input
						id="countInput" class="weui-count__number" readonly="readonly"
		...
		<div class="weui-cell">
			...
			<div class="weui-cell__bd">
				...
				<input class="weui-input" id="select-city-value" value=""
					type="hidden">
			</div>
		</div>
		...
		<div class="weui-cell">
			...
			<div class="weui-cell__ft text-dark" id="totalPriceDiv">0.00 元</div>
		</div>
	</div>
	...
	<div class="p-l-15 p-r-15">
		<a href="javascript:void(0);" onclick="buyAction();"
			class="weui-btn weui-btn_warn">购卡</a>
	</div>
		...
		<div class="weui-popup__modal">
			...
			<div class="modal-content">
				<div class="weui-flex flex-wrap text-center">
					<div class=" card-item">
						<img src="<%=basePath%>/pages/images/demo-card-01.png" width="90%"
							class="img-round card-select-on" />
					</div>
					<div class=" card-item">
						<img src="<%=basePath%>/pages/images/demo-card-02.png" width="90%"
							class="img-round" />
					</div>
					<div class=" card-item">
						<img src="<%=basePath%>/pages/images/demo-card-03.png" width="90%"
							class="img-round" />
					</div>
					<div class=" card-item">
						<span class="weui-badge delect-card"
							style="position: absolute; top: -.4em; right: .4em; z-index: 9999; padding: 4px 8px;">删除</span>
						<img src="<%=basePath%>/pages/images/demo-card-04.png" width="90%"
							class="img-round" />
					</div>
	...
	<div class="custom-card-popup" id="tailor_page">
		...
		<div class="contaniner p-15">
			<div id="clipArea"
				style="user-select: none; overflow: hidden; position: relative; padding: 15px;">
			...
			<div class="weui-flex m-t-10">
				<div class="weui-flex__item p-r-10">
					...
					<button type="button"
						class="weui-btn weui-btn_plain-primary btn-select-file">选择文件</button>
				</div>
			...
			<div id="viewDiv">
				<div id="view"
					style="background-repeat: no-repeat; background-position: center center; background-size: contain;"></div>
			</div>


			<canvas class="display-none" id="card_bkImg"></canvas>
			<img class="view-card m-t-20" id="sss"
				src="<%=basePath%>/pages/images/demo-card-01.png"
				style="display: none"> <a
				class="weui-btn weui-btn_primary display-none m-t-10"
				href="javascript:;" id="clipArea_sure">确定</a>
	...
	<script type="text/javascript"
		src="<%=basePath%>pages/js/hammer.min.js"></script>
	<script type="text/javascript"
	...
	<script type="text/javascript">
		...
		function buyAction() {
			...
			window.location.href = "orderAdd?count=" + count + "&type=1&jv="
					...
					+ "&priceSale=" + priceSale + "&idGoods=" + idGoods;
		}
	</script>
	...
	<script>
		...
		$(".weui-payselect-li").on('click', function() {
			...
			var param = $(this).find('input').val().split('-');
			idSku = param[0];
			price = param[1];
			...
			// 计算总金额
			$("#totalPriceDiv").html(eval('param[1]*$("#countInput").val()')+" 元")
			
			
		});

		// 输入金额时，清空金额选择
		$("#priceSaleInput").on('input', function() {
			$(".weui-payselect-li").find('a').removeClass("weui-payselect-on");
			
			price = "";
			
			// 计算总金额
			$("#totalPriceDiv").html(eval('$("#priceSaleInput").val()*$("#countInput").val()')+" 元")
		})
		...
		$('.weui-count__decrease').click(
				function(e) {
					...
					// 计算总金额
					if (price != "") {
						$("#totalPriceDiv").html(eval('price*$("#countInput").val()')+" 元")
					} else {
						$("#totalPriceDiv").html(eval('$("#priceSaleInput").val()*$("#countInput").val()')+" 元")
					}
				})
						...
						function() {
							...
							if ($(this).hasClass("weui-btn_primary")) {
								...
								formData.append("image",
										convertBase64UrlToBlob(cardURL));
